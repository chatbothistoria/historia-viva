...
# ═══════════════════════════════════════════════════════════════════════════════
def get_era_image_b64(era_id: str) -> str:
    """
    Lee la imagen local de la época y la devuelve en base64 para incrustarla
    en el HTML sin depender de rutas externas ni de la API de Wikipedia.
    Devuelve "" si el archivo no existe.
    """
    import base64
    img_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "images", f"{era_id}.jpg")
...
def load_era_images() -> dict:
    """
    Carga todas las imágenes locales al arrancar y las cachea en memoria.
    Cada imagen se almacena como data URI base64 para incrustarla directamente
    en el HTML — sin peticiones externas, sin problemas de rutas en Streamlit Cloud.
    """
    images = {}
    for era_id in ERA_META:
        b64 = get_era_image_b64(era_id)
        if b64:
            images[era_id] = b64
    return images
...
# ═══════════════════════════════════════════════════════════════════════════════
# Límite diario conservador (por debajo del límite real de Gemini)
# Gemini 2.5 Flash free tier: ~500 RPD. Usamos 400 como margen de seguridad.
GEMINI_DAILY_LIMIT = 400
...
def local_direct_answer(question: str, era_data: dict, level: str) -> str:
    """
    Responde directamente desde la base de datos cuando el score es alto (≥3).
    NO llama a Gemini → ahorra quota gratuita.
    Devuelve el texto de la FAQ o "" si no hay coincidencia suficiente.
    """
...
Responde SIEMPRE en primera persona, como ese personaje histórico que habla directamente con el alumno.
...
=== REGLAS GENERALES ===
1. ROL: Nunca rompas el personaje. Eres esa persona histórica, no un narrador externo.
   Habla desde tu experiencia vivida, no como un libro de texto.
...
2. PREGUNTAS ANACRÓNICAS (coches, móviles, internet, aviones, ordenadores, TV...):
   ...
   Explica qué se usaba en tu época para lo mismo, adaptado al nivel de edad.

3. NOMBRES Y FECHAS EXACTOS: Si no aparecen en tu contexto, dilo en personaje:
   "no recuerdo exactamente cómo se llamaba..." NUNCA inventes datos concretos.

4. TEMAS SENSIBLES (violencia, muerte, represión, hambre):
   Responde con honestidad apropiada para la edad. Sin morbo ni detalle gráfico.
   En nivel Infantil: suaviza o redirige. En niveles superiores: trata con respeto y contexto.

5. LONGITUD MÁXIMA: {level_cfg['max_chars']} caracteres. Respeta estrictamente el límite.

6. IDIOMA: Siempre en español. Sin asteriscos, viñetas ni formato markdown.
   Texto corrido, natural, como si hablaras en voz alta."""
...
def handle_question(question: str, current_era: dict, level: str,
    ...
    """
    Lógica híbrida:
      1. FAQ con coincidencia clara (score≥3) → respuesta local directa.
         No llama a Gemini → preserva la quota gratuita.
      2. Resto → Gemini con streaming.
         Comprueba el límite diario antes de llamar.
    ...
    # ── Intento de respuesta local (no consume quota) ────────────────────────
    direct = local_direct_answer(question, current_era, level)

    if direct:
        # Respuesta local: instantánea, sin coste, sin límite
        with st.chat_message("assistant", avatar=era_meta["emoji"]):
            st.write(direct)
        full_text = direct
        source    = "local"
    else:
        # Comprobar si hemos superado el límite diario de seguridad
        calls_today = gemini_calls_today()
        if calls_today >= GEMINI_DAILY_LIMIT:
            fallback = (
                "Lo siento, hemos llegado al límite de preguntas de inteligencia "
                "artificial de hoy. El límite se reinicia a medianoche. "
                "Puedes seguir explorando las sugerencias de preguntas del menú lateral."
            )
            with st.chat_message("assistant", avatar=era_meta["emoji"]):
                st.warning(fallback)
            full_text = fallback
            ...
            # Respuesta con Gemini (streaming)
            with st.chat_message("assistant", avatar=era_meta["emoji"]):
...
def render_teacher_panel():
    ...
    <div class="teacher-stat-card" style="border-color:{quota_color}40;">
      ...
      <div style="font-size:.78rem;color:#667685;margin-top:4px;">
        {remaining} llamadas disponibles · Las respuestas con FAQ local no consumen quota
      </div>
    </div>
    """, unsafe_allow_html=True)
...
def main():
    ...
    with st.sidebar:
        ...
        if new_level != level:
            ...
            ss.big_text = (new_level == "infantil")
            st.rerun()
    ...
    if current_era:
        ...
        # Texto de apertura adaptado al nivel — se lee del diccionario Python
        # APERTURA_NIVEL para garantizar que funciona independientemente
        # de la caché de @st.cache_resource y del estado de los archivos JS.
        level_key = (level
